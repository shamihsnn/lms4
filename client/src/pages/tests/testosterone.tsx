...
export default function TestosteroneTest() {
  ...
  const createTestMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Testosterone test saved" });
    }
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    testosteroneParameters.forEach(p => {
    ...
    testosteroneParameters.forEach(p => (normalRanges[p.name] = `${p.normalRange} ${p.unit}`));

    await createTestMutation.mutateAsync({
      testId: formData.testId,
      patientId: patient.id,
      testType: "Testosterone",
  ...
  const handlePrint = () => {
    const patient = patients.find(p => p.patientId === formData.patientId);
    const rows: ReportRow[] = testosteroneParameters.map(p => {
    ...
    printLabReport({ reportTitle: "FINAL REPORT", testId: formData.testId, testType: "Testosterone", patient, rows, comments: formData.comments, minimal: true });
  };

  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800">Testosterone</h1>
        <p className="text-slate-600">Adult reference ranges for male and female</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>New Testosterone Test</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {testosteroneParameters.map(param => (
                <div key={param.name}>
