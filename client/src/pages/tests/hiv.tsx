...
export default function HIVTest() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const normalRanges: Record<string, string> = { qualitative: "Non-reactive" };
    await createTestMutation.mutateAsync({ testId: formData.testId, patientId: patient.id, testType: "HIV (Rapid)", testResults: { qualitative: formData.qualitative }, normalRanges, flags, status: "completed", performedBy: undefined, modifiedBy: undefined });
  };

  const handlePrint = () => {
    const patient = patients.find(p => p.patientId === formData.patientId);
    const value = formData.qualitative || "";
    const flag: ReportRow["flag"] = value ? (value === "Non-reactive" ? "NORMAL" : "ABNORMAL") : "";
    const rows: ReportRow[] = [{ parameterLabel: "HIV 1/2 (Qualitative)", value, normalRange: "Non-reactive", flag }];
    printLabReport({ reportTitle: "FINAL REPORT", testId: formData.testId, testType: "HIV (Rapid)", patient, rows, comments: formData.comments, minimal: true });
  };

  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800">HIV (AIDS) Rapid Test</h1>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>New HIV Test</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div>
                <Label className="block text-sm font-medium text-slate-700 mb-2">HIV 1/2 (Qualitative)</Label>
