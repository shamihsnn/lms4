...
const stoolParams = [
  { name: "color", label: "Color", unit: "", normalRange: "Brown", type: "text" as const },
  { name: "consistency", label: "Consistency", unit: "", normalRange: "Formed", type: "text" as const },
  { name: "mucus", label: "Mucus", unit: "", normalRange: "Absent", type: "text" as const },
  { name: "blood", label: "Visible Blood", unit: "", normalRange: "Absent", type: "text" as const },
  { name: "ovaCysts", label: "Ova/Cysts", unit: "", normalRange: "Not seen", type: "text" as const },
  { name: "parasites", label: "Parasites", unit: "", normalRange: "None detected", type: "text" as const },
  { name: "occultBlood", label: "Occult Blood", unit: "", normalRange: "Negative", type: "text" as const },
  { name: "reducingSubstances", label: "Reducing Substances", unit: "", normalRange: "Negative", type: "text" as const },
  { name: "ph", label: "pH", unit: "", normalRange: "6.5-7.5", type: "number" as const, step: "0.1" },
];

export default function StoolRETest() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    stoolParams.forEach(p => {
      ...
      if (p.type === "number") {
        ...
        flags[p.name] = val.toLowerCase() === p.normalRange.toLowerCase() ? "NORMAL" : "ABNORMAL";
      }
    });
    const normalRanges: Record<string, string> = {};
    stoolParams.forEach(p => (normalRanges[p.name] = p.normalRange));
  ...
  const handlePrint = () => {
    const patient = patients.find(p => p.patientId === formData.patientId);
    const rows: ReportRow[] = stoolParams.map(p => {
      ...
      if (value) {
        if (p.type === "number") {
          ...
          flag = value.toLowerCase() === p.normalRange.toLowerCase() ? "NORMAL" : "ABNORMAL";
        }
      }
      return { parameterLabel: p.label, value, unit: p.unit || undefined, normalRange: p.normalRange, flag };
    });
    printLabReport({ reportTitle: "FINAL REPORT", testId: formData.testId, testType: "Stool Routine Examination", patient, rows, comments: formData.comments, minimal: true });
  };

  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800">Stool Routine Examination (R/E)</h1>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>New Stool R/E</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {stoolParams.map(param => (
                <div key={param.name}>
                  <Label className="block text-sm font-medium text-slate-700 mb-2">{param.label}</Label>
                  {param.type === "number" ? (
                    <div className="flex">
                      <Input type="number" step={param.step} value={formData.results[param.name] || ""} onChange={(e) => setFormData(p => ({ ...p, results: { ...p.results, [param.name]: e.target.value } }))} className="w-full" placeholder={param.normalRange} />
                    </div>
                  ) : (
