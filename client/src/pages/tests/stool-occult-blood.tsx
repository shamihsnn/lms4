...
export default function StoolOccultBloodTest() {
  const [formData, setFormData] = useState({ testId: "", patientId: "", result: "", comments: "" });
  ...
  const createTestMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      setFormData({ testId: "", patientId: "", result: "", comments: "" });
      toast({ title: "Stool occult blood saved" });
    }
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const flags = { occultBlood: formData.result ? (formData.result.toLowerCase() === "negative" ? "NORMAL" : "ABNORMAL") : "" } as Record<string, string>;
    const normalRanges = { occultBlood: "Negative" } as Record<string, string>;

    await createTestMutation.mutateAsync({
      testId: formData.testId,
      patientId: patient.id,
      testType: "Stool Occult Blood",
      testResults: { occultBlood: formData.result },
      normalRanges,
      flags,
      status: "completed",
  ...
  const handlePrint = () => {
    const patient = patients.find(p => p.patientId === formData.patientId);
    const value = formData.result || "";
    const flag: ReportRow["flag"] = value ? (value.toLowerCase() === "negative" ? "NORMAL" : "ABNORMAL") : "";
    const rows: ReportRow[] = [{ parameterLabel: "Stool Occult Blood", value, normalRange: "Negative", flag }];
    printLabReport({ reportTitle: "FINAL REPORT", testId: formData.testId, testType: "Stool Occult Blood", patient, rows, comments: formData.comments, minimal: true });
  };

  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800">Stool for Occult Blood</h1>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>New Stool Occult Blood Test</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div>
                ...
                <Select value={formData.result} onValueChange={(v) => setFormData(p => ({ ...p, result: v }))}>
