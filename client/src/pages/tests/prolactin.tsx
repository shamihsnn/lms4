...
export default function ProlactinTest() {
  ...
  const createTestMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Prolactin test saved", description: "Prolactin results recorded" });
    },
  });
  ...
  const getFlag = (paramName: string, value: string) => {
    ...
    const p = prolactinParameters.find(x => x.name === paramName);
    if (!p) return "";
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    prolactinParameters.forEach(p => (normalRanges[p.name] = `${p.normalRange} ${p.unit}`));
  ...
  const handleResultChange = (name: string, value: string) => setFormData(prev => ({ ...prev, results: { ...prev.results, [name]: value } }));
  const handleIdUpdate = async (newId: string) => { setFormData(prev => ({ ...prev, testId: newId })); setEditingTestId(false); };

  const handlePrint = () => {
    const patient = patients.find(p => p.patientId === formData.patientId);
    const rows: ReportRow[] = prolactinParameters.map(p => {
    ...
    printLabReport({ reportTitle: "FINAL REPORT", testId: formData.testId, testType: "Prolactin", patient, rows, comments: formData.comments, minimal: true });
  };

  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800">Prolactin</h1>
        <p className="text-slate-600">Gender/physiologic state-specific reference ranges</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>New Prolactin Test</CardTitle>
        </CardHeader>
        <CardContent>
