...
const parameter = { name: "amylase", label: "Serum Amylase", unit: "U/L", normalRange: "30-110" } as const;

export default function AmylaseTest() {
  ...
  const createTestMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "Amylase test saved", description: "Serum Amylase recorded" });
    },
  });
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800">Serum Amylase</h1>
        <p className="text-slate-600">Adults (All sexes): 30 - 110 U/L (Standard adult range)</p>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>New Amylase Test</CardTitle>
        </CardHeader>
        <CardContent>
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              <div>
                ...
                <div className="flex">
                  <Input type="number" step="1" value={formData.results[parameter.name] || ""} onChange={(e) => setFormData((p) => ({ ...p, results: { ...p.results, [parameter.name]: e.target.value } }))} className="flex-1 rounded-r-none" placeholder={parameter.normalRange} />
                ...
                <p className="text-xs text-slate-500 mt-1">Normal (Adults, all sexes): {parameter.normalRange} {parameter.unit}</p>
              </div>
            </div>
            <div>
