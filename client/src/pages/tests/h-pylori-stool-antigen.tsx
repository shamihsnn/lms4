...
export default function HPyloriStoolAntigenTest() {
  ...
  const createTestMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "H. pylori Stool Antigen test saved" });
    }
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await createTestMutation.mutateAsync({ testId: formData.testId, patientId: patient.id, testType: "H. pylori Stool Antigen (Qualitative)", testResults: { qualitative: formData.qualitative }, normalRanges, flags, status: "completed", performedBy: undefined, modifiedBy: undefined });
  };
  ...
  const handlePrint = () => {
    ...
    const rows: ReportRow[] = [
      { 
        parameterLabel: "H. pylori Antigen", 
        value, 
        unit: "", 
    ...
    printLabReport({ 
      ...
      testType: "H. pylori Stool Antigen", 
      patient, 
      rows, 
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800">H. pylori Stool Antigen</h1>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>New H. pylori Stool Antigen Test</CardTitle>
        </CardHeader>
        <CardContent>
