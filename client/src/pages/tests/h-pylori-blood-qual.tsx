...
export default function HPyloriBloodQualTest() {
  ...
  const createTestMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({ title: "H. pylori (Blood, Qualitative) test saved" });
    }
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    await createTestMutation.mutateAsync({ 
      ...
      testType: "H. pylori (Blood, Qualitative)", 
      testResults: { qualitative: formData.qualitative }, 
      normalRanges, 
      flags, 
      status: "completed", 
  ...
  const handlePrint = () => {
    ...
    const rows: ReportRow[] = [
      { parameterLabel: "H. pylori Antibody (Qualitative)", value, unit: "", normalRange: "Negative", flag }
    ];
    printLabReport({ reportTitle: "FINAL REPORT", testId: formData.testId, testType: "H. pylori (Blood, Qualitative)", patient, rows, comments: formData.comments, minimal: true });
  };

  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-slate-800">H. pylori (Blood, Qualitative)</h1>
      </div>
      <Card>
        <CardHeader>
          <CardTitle>New H. pylori (Blood, Qualitative)</CardTitle>
        </CardHeader>
        <CardContent>
